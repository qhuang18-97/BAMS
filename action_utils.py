...
def prey_select_action(action_out):
    action_out =action_out.expand(1, 5)
    action_prob = [z.exp() for z in action_out]
    ret = torch.stack([torch.multinomial(x, 1).detach() for x in action_prob])
    return ret
